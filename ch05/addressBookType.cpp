...
addressBookType::~addressBookType()
{   // Print details of each person in addressBook to console.
...
void addressBookType::printDetails(string first, string last) const
{
    ...
    listIt = addressList.begin();
    while (listIt != addressList.end()) {
        if (listIt->getFirstName() == first && 
                listIt->getLastName() == last)
            listIt->print();
        listIt++;
    }
}
                                ...
                                const dateType& date2) const
{
    ...
    listIt = addressList.begin();
    while (listIt != addressList.end()) {
        if (month1 < listIt->getMonth() && 
            month2 > listIt->getMonth()) {
            cout << listIt->getFirstName() << " "
                 << listIt->getLastName() << endl;
