...
template <class Type> class headerTrailerLinkedList {
public:
  headerTrailerLinkedList<Type> &
  operator=(const headerTrailerLinkedList<Type> &otherList);
  // Overloaded assignment operator.
  // Postcondition: Calling object assigned elements from otherList.

  bool operator!=(const headerTrailerLinkedList<Type> &otherList) const;
...
template <class Type>
bool headerTrailerLinkedList<Type>::operator!=(
    const headerTrailerLinkedList<Type> &otherList) const {
...
template <class Type>
headerTrailerLinkedList<Type>::headerTrailerLinkedList(Type minVal,
                                                       Type maxVal) {
...
template <class Type> void headerTrailerLinkedList<Type>::initializeList() {
  destroyList();
}

template <class Type> void headerTrailerLinkedList<Type>::destroyList() {
...
template <class Type> void headerTrailerLinkedList<Type>::print() const {
  nodeType<Type> *current; // Pointer to traverse the list
...
template <class Type> int headerTrailerLinkedList<Type>::length() const {
  return count;
}
...
void headerTrailerLinkedList<Type>::insertNode(const Type &insertItem) {
  ...
  if (first->link == last) // If list is empty, newNode is the only node
  {
    ...
    if (current == first->link) // Insert newNode after header
    {
      ...
      first->link = newNode;
      count++;
    } else { // Inset newNode between trailCurrent and current
      if (current != last) {
...
template <class Type>
void headerTrailerLinkedList<Type>::copyList(
    const headerTrailerLinkedList<Type> &otherList) {
  nodeType<Type> *newNode;      // Pointer to create a node
  nodeType<Type> *current;      // Pointer to traverse the list
  ...
  if (otherList.first->link == otherList.last) // otherList is empty
  {
    ...
    otherCurrent =
        otherCurrent->link; // Make otherCurrent point to the next node
