...
void hashT<elemType>::search(int &hashIndex, const elemType &rec,
  ...
  while (
      (indexStatusList[hashIndex] == 1 || indexStatusList[hashIndex] == -1) &&
      HTable[hashIndex] != rec) {
    hashIndex = (hashIndex + inc) % HTSize;
    inc = inc + 2;
  }
...
void hashT<elemType>::retrieve(int hashIndex, elemType &rec) const {
  if (indexStatusList[hashIndex] == 1)
    ...
    std::cerr << "Error: no item stored at position " << hashIndex << ".\n";
}
...
template <class elemType> void hashT<elemType>::print() const {
  for (int i = 0; i < HTSize; ++i)
    if (indexStatusList[i] == 1)
      std::cout << i << ": " << HTable[i] << '\n';
}

template <class elemType> hashT<elemType>::hashT(int size) : HTSize(size) {
  HTable = new elemType[HTSize];
  indexStatusList = new int[HTSize];
  length = 0;
}

template <class elemType> hashT<elemType>::~hashT() {
  delete[] HTable;
