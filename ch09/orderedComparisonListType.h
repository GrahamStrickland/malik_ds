...
public:
  int binarySearch(const elemType &item, int &numComparisons) const;
  ...
  int binarySeqSearch(const elemType &item, int &numComparisons) const;
  // Function to search the list for item using a binary
  ...
  int recursiveBinarySearch(const elemType &item, int first, int last) const;
  ...
  int seqOrdSearch(const elemType &item, int first, int last) const;
...
int orderedComparisonListType<elemType>::binarySeqSearch(
  ...
  while (first <= last && !found && (last - first) > MIN_SEARCH_SIZE) {
    mid = (first + last) / 2;
...
template <class elemType>
int orderedComparisonListType<elemType>::seqOrdSearch(const elemType &item,
                                                      int first,
                                                      int last) const {
