...
int main() { // Test constructors.
  int ID, hashIndex, studentHashSize(13), numStudents(8);
  ...
  for (int i = 0; i < numStudents; ++i) {
    std::cout << "Please enter ID of student " << std::to_string(i + 1) << ": ";
    std::cin >> ID;
  ...
  while (ID != -1) {
    ...
    std::cout << (found ? "ID " + std::to_string(ID) + " found at position " +
                              std::to_string(hashIndex)
  ...
  std::cin >> ID;
  std::cout << ID
            << (studentID.isItemAtEqual(hashIndex, ID) ? " is equal to"
                                                       : " is not equal to")
            << " item at "
            << "position " << hashIndex << ".\nItem stored at "
            << "position " << hashIndex << ": ";
  ...
  std::cin >> ID;
  std::cout << "Please enter the position of student ID " << ID << ": ";
  std::cin >> hashIndex;
...
int hashFunction(int k, int hashSize) { return k % hashSize; }