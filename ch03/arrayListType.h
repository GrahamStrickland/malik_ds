...
public:
  ...
  // Postcondition: If removeItem is found in the list,
  //   all occurrences of that item are removed from the list,
  //   and length is decremented by the number of occurrences.
  elemType min() const;
  // Function to return the minimum value in the list.
  // Postcondition: Minimum value of type elemType returned.
  elemType max() const;
  // Function to return the maximum value in the list.
  // Postcondition: Maximum value of type elemType returned.
...
void arrayListType<elemType>::removeAll(const elemType &removeItem) {
  ...
  if (length == 0)
    ...
    while ((loc = seqSearch(removeItem)) >= 0)
      removeAt(loc);
  }
} // end removeAll
...
template <class elemType> elemType arrayListType<elemType>::max() const {
  assert(length != 0);
