...
void addressBookType::matchDate(
    const dateType &date1,
    const dateType &date2) const { // Extract values from dateType objects.
  ...
  for (unsigned int i = 0; i < length; i++) {
    if (month1 < list[i].getMonth() && month2 > list[i].getMonth()) {
...
void addressBookType::matchDate(
    int month) const { // Search list for matching entries and print.
  for (unsigned int i = 0; i < length; i++)
    if (list[i].getMonth() == month)
      cout << list[i].getFirstName() << " " << list[i].getLastName() << endl;
}

void addressBookType::matchStatus(string rel)
    const { // Check that rel is a valid string from the available options.
