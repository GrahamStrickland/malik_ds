...
int matrixType::operator()(
    int rowRef, int colRef) const { // Check that rowRef and colRef are valid.
  assert(rowRef >= 0 && rowRef < rows && colRef >= 0 && colRef < cols);
...
int &matrixType::operator()(
    int rowRef, int colRef) { // Check that rowRef and colRef are valid.
  assert(rowRef >= 0 && rowRef < rows && colRef >= 0 && colRef < cols);
...
const matrixType &matrixType::operator=(
    const matrixType &rightMatrix) { // Check that dimensions match rightMatrix.
  assert(rows == rightMatrix.rows && cols == rightMatrix.cols);
...
matrixType matrixType::operator+(
    const matrixType &rightMatrix) { // Check that dimensions match.
  assert(rows == rightMatrix.rows && cols == rightMatrix.cols);
  ...
  for (int row = 0; row < rows; row++)
    for (int col = 0; col < cols; col++)
      temp.matrix[row][col] = matrix[row][col] + rightMatrix.matrix[row][col];
...
int matrixType::getRows() const { return rows; }

int matrixType::getCols() const { return cols; }

matrixType matrixType::operator*(
    const matrixType &rightMatrix) { // Check that inner dimensions match.
  ...
  for (int row = 0; row < rows; row++)
    for (int col = 0; col < rightMatrix.cols; col++)
      for (int entry = 0; entry < cols; entry++)
        temp.matrix[row][col] +=
            matrix[row][entry] * rightMatrix.matrix[entry][col];

  return temp;
}

matrixType::matrixType(int numRows, int numCols) { // Assign rows and columns
  rows = numRows;
  cols = numCols;
...
matrixType::matrixType(
    const matrixType
        &rightMatrix) { // Assign rows and columns from rightMatrix.
...
matrixType::~matrixType() { // Deallocate memory for each row, then rows.
  for (int row = 0; row < rows; row++)
    delete[] matrix[row];
  delete[] matrix;
}
