...
template <class elemType> class safeArray {
public:
...
template <class elemType> safeArray<elemType>::safeArray(int first, int last) {
  assert(first < last);
  firstIndex = first;
  lastIndex = last;
...
template <class elemType>
const safeArray<elemType> &
safeArray<elemType>::operator=(const safeArray &otherArr) {
  if (this != &otherArr) {
    delete[] arr;
    firstIndex = otherArr.getFirstIndex();
    lastIndex = otherArr.getLastIndex();
...
template <class elemType> safeArray<elemType>::~safeArray() { delete[] arr; }

template <class elemType> elemType &safeArray<elemType>::operator[](int index) {
  assert(firstIndex <= index && index <= lastIndex);
  int arrRef = index - firstIndex;
  return arr[arrRef];
}

template <class elemType> int safeArray<elemType>::getLength() const {
  return length;
}

template <class elemType> int safeArray<elemType>::getFirstIndex() const {
  return firstIndex;
}

template <class elemType> int safeArray<elemType>::getLastIndex() const {
  return lastIndex;
}
