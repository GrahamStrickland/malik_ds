...
template <class Type> class queueType : public queueADT<Type> {
public:
...
template <class Type> bool queueType<Type>::isEmptyQueue() const {
  return queueFront == queueRear;
} // end isEmptyQueue

template <class Type> bool queueType<Type>::isFullQueue() const {
  return (queueFront == queueRear + 1) ||
         (queueFront == 0 && queueRear == maxQueueSize - 1);
} // end isFullQueue

template <class Type> void queueType<Type>::initializeQueue() {
  queueFront = 0;
  queueRear = 0;
} // end initializeQueue

template <class Type> Type queueType<Type>::front() const {
  assert(!isEmptyQueue());
  return list[(queueFront + 1) % maxQueueSize];
} // end front

template <class Type> Type queueType<Type>::back() const {
  assert(!isEmptyQueue());
  return list[queueRear];
} // end back

template <class Type> void queueType<Type>::addQueue(const Type &newElement) {
  if (!isFullQueue()) {
    queueRear =
        (queueRear + 1) % maxQueueSize; // use the
...
template <class Type> void queueType<Type>::deleteQueue() {
  if (!isEmptyQueue())
    queueFront =
        (queueFront + 1) % maxQueueSize; // use the
...
template <class Type> queueType<Type>::queueType(int queueSize) {
  if (queueSize <= 0) {
