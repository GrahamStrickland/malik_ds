...
void runSimulation() {
  ...
  int simulationTime, numOfServers, transTime, tBetweenCArrival,
      numOfCustomers = 0, clock = 0, totalWaitingTime = 0, numOfCustArrived = 0,
      numOfCustServed = 0, numOfCustInQueue = 0, numOfCustAtServers = 0,
      serverID, served = 0;
  double randomNumber = 0.0, avgWaitingTime = 0.0;
  setSimulationParameters(simulationTime, numOfServers, transTime,
                          tBetweenCArrival);
  ...
  for (clock = 1; clock <= simulationTime; clock++) {
    ...
    if (randomNumber > exp(-(1.0 / static_cast<double>(tBetweenCArrival)))) {
      ...
      queue.push(cust);
      cout << "Customer number " << numOfCustomers + 1
  ...
  cout.precision(2);
  cout << endl
       << "The simulation ran for " << simulationTime << " time units" << endl
       ...
       << "Number of customers left at the servers: " << numOfCustAtServers
       << endl
