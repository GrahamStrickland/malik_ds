...
public:
    bool operator==(const stackType<Type>&);
        //Overload the relational equality operator.
    ...
    void reverseStack(stackType<Type> &otherStack);
        //Function to copy the elements of calling stack onto otherStack
        //  in reverse order.
        //Postcondition: otherStack has only the elements of calling stack
...
void stackType<Type>::reverseStack(stackType<Type> &otherStack)
{
    ...
    int current = stackTop - 1;

    while (list[current] >= 0) {
        otherStack.push(list[current]);
        current--;
    }
